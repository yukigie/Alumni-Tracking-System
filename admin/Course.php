...
if (isset($_POST['insertdata'])) {
    ...
    $check = mysqli_query($con, "SELECT * FROM tbl_courses WHERE Course_Name='$Course'");
    
    if (mysqli_num_rows($check) > 0) {
        setAlert("Already Exist!", "The Course Name You Entered Is Already On The List!", "warning");
    } else {

        $query = "INSERT INTO tbl_courses (Course_Name, Units) VALUES ('$Course', '$Unit')";
                
                if (mysqli_query($con, $query)) {
                    setAlert("Successfully Added!", "A New Course Has Been Successfully Added!", "success");
...
if (isset($_POST['updatedata'])) {
    ...
    $UP_Unit = $_POST['UP_Unit'];
    
    // Check if the Course already exists in another record
    $check = mysqli_query($con, "SELECT * FROM tbl_courses WHERE Course_Name='$UP_Course' AND Course_ID != '$ID'");
    
    if (mysqli_num_rows($check) > 0) {
        setAlert("Already Exist!", "The Course Name You Entered Is Already On The List!", "warning");
    } else {
       
           $query = "UPDATE tbl_courses SET Course_Name = '{$UP_Course}', Units = '{$UP_Unit}' WHERE Course_ID = '{$ID}'";
            
            if (mysqli_query($con, $query)) {
                setAlert("Successfully Updated!", "Course has been successfully updated!", "success");
                header("Location: Course.php");
                exit;
            } else {
                setAlert("Data Not Updated!", "Failed to update the Course.", "error");
                header("Location: Course.php");
                exit;
            }
        }
...
if (isset($_POST['deletedata'])) {
    ...
    $sql = "DELETE FROM tbl_courses WHERE Course_ID=$Course_ID";
    
    if ($con->query($sql)) {
        setAlert("Successfully Deleted!", "Course Is Successfully Deleted!", "success");
        header("Location: Course.php");
        exit;
    } else {
        setAlert("Data Not Deleted!", "Failed to delete the Course.", "error");
        header("Location: Course.php");
        exit;
    }
}
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="exampleModalLabel" style="font-weight: 600;">Add Course</h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <h5>Course Details</h5>

      <form action="Course.php" method="POST" enctype="multipart/form-data" class="form_body" autocomplete="off">
        ...
        <div class="modal-body">
            ...
            <div class="col-lg-12">
              <div class="form-group" style="margin-top: 20px;">
                ...
                <input type="number" name="Unit" id="Unit" class="form-control" placeholder="Enter Number of Units" min="1" required>
              </div>
            </div>
          </div>
        </div>
...
<div class="modal fade" id="EditModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="exampleModalLabel" style="font-weight: 600;">Update Course</h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
        ...
        <div class="modal-body">
          ...
          <div class="row justify-content-center">
            <div class="col-lg-12">
              <div class="form-group" style="margin-top: 20px;">
                <label>Course Name:</label>
                <input type="text" name="UP_Course" id="UP_Course" class="form-control" placeholder="Enter Course Name" required>
              </div>
            </div>
            ...
            <div class="col-lg-12">
              <div class="form-group" style="margin-top: 20px;">
                <label>No. of Units:</label>
                <input type="number" name="UP_Unit" id="UP_Unit" class="form-control" placeholder="Enter Number of Units" min="1" required>
              </div>
            </div>
          </div>
        </div>
...
<div class="modal fade" id="DeleteModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="exampleModalLabel" style="font-weight: 600;">Delete Course</h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <h5>Confirm Message</h5>

      <form action="Course.php" method="POST" class="form_body" autocomplete="off">
        <div class="modal-body">
