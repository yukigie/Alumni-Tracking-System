...
<head>
    ...
    <title>Alumni | Job Opening</title>
...
<div class="modal fade" id="JobModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      ...
      <div class="modal-body">
        ...
        <div class="row">
            ...
            <div class="col-lg-6" id="coltbtn">
                <button class="applybtn" type="submit" name="applybtn" onclick="submitNotifForm()">APPLY NOW</button>
 ...
 <form action="Alumni-Notif.php" method="POST" id="notif-form" class="notif-form">

        <input type="hidden" name="sender_mail" id="sender_mail" class="form-control" value="gsample219@gmail.com">

        <input type="hidden" name="emp_email1" id="emp_email1">
...
<!-- Search Bar -->
    <div class="input-group mb-4">
        ...
        <input type="text" id="searchInput" class="form-control" oninput="toggleClearButton()" placeholder="Search for Job Titles, Keywords, and Company">

        <div class="search-icon" onclick="filterJobs()">
            <i class="fa fa-search" aria-hidden="true"></i>
        </div>

        <!-- Clear Icon (Hidden initially) -->
        <span id="clearButton" class="input-group-text" onclick="clearSearch()" style="display: none; cursor: pointer; position: absolute; right: 55px; top: 3px; z-index: 5; background: transparent; border: none; color: gray;">
...
<div class="col-lg-12">
    ...
    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
          <li class="nav-item" role="presentation">
            <button class="nav-tab active" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-home" type="button" role="tab" aria-controls="pills-home" aria-selected="true" onclick="setActive(this)">Jobs For You</button>
          </li>
          <li class="nav-item" role="presentation">
            <button class="nav-tab" id="pills-profile-tab" data-bs-toggle="pill" data-bs-target="#pills-profile" type="button" role="tab" aria-controls="pills-profile" aria-selected="false" onclick="setActive(this)">Recent Jobs</button>
          </li>
        </ul>
        <div class="tab-content" id="pills-tabContent">
          <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab" tabindex="0">
            <p>Jobs based on your profile & job preference</p>
            <div class="row" id="jobsForYou">
            <?php

            $query = "SELECT DISTINCT ej.* 
                    FROM tbl_employer_joblist ej
                    LEFT JOIN tbl_employer emp ON ej.Company_ID = emp.Employer_ID
                    LEFT JOIN tbl_alumni_jobpreftype ajt ON ej.Job_Type = ajt.Job_Type
                    LEFT JOIN tbl_alumni a ON (
                        (ej.State = a.State AND 
                        ej.Postal = a.Postal AND 
                        ej.City = a.City)
                        OR (
                            -- Compare Course with the part before the hyphen in Industry
                            TRIM(LOWER(a.Course)) LIKE CONCAT('%', TRIM(LOWER(SUBSTRING_INDEX(emp.Industry, '-', 1))), '%')
                            -- Compare Course with the part after the hyphen in Industry
                            OR TRIM(LOWER(a.Course)) LIKE CONCAT('%', TRIM(LOWER(SUBSTRING_INDEX(emp.Industry, '-', -1))), '%')
                            -- Compare the entire Industry value with Course
                            OR TRIM(LOWER(emp.Industry)) LIKE CONCAT('%', TRIM(LOWER(a.Course)), '%')
                            OR TRIM(LOWER(a.Course)) LIKE CONCAT('%', TRIM(LOWER(emp.Industry)), '%')
                        )
                    )
                    LEFT JOIN tbl_employer_jobsched ejs ON ej.ID = ejs.Job_ID
                    LEFT JOIN tbl_alumni_jobprefdays ajd ON ejs.Day = ajd.Days
                    LEFT JOIN tbl_alumni_jobprefshift ajs ON ejs.Shift = ajs.Shift_Name
                    LEFT JOIN tbl_alumni_jobpreftype ajtt ON ej.Job_Type = ajtt.Job_Type
                    LEFT JOIN tbl_alumni_jobprefsched ajts ON ejs.Type = ajts.Schedule
                    LEFT JOIN tbl_alumni_jobprefsalary ajsal ON ej.Salary = ajsal.Salary
                    WHERE 
                        (
                            ajt.Job_Type IS NOT NULL
                            OR (ej.State = a.State AND ej.City = a.City AND ej.Postal = a.Postal)
                            OR ajd.Days IS NOT NULL
                            OR ajs.Shift_Name IS NOT NULL
                            OR ajtt.Job_Type IS NOT NULL
                            OR ajts.Schedule IS NOT NULL
                            OR ajsal.Salary IS NOT NULL
                            -- Compare entire Industry or the parts before/after hyphen with Course
                            OR TRIM(LOWER(a.Course)) LIKE CONCAT('%', TRIM(LOWER(SUBSTRING_INDEX(emp.Industry, '-', 1))), '%')
                            OR TRIM(LOWER(a.Course)) LIKE CONCAT('%', TRIM(LOWER(SUBSTRING_INDEX(emp.Industry, '-', -1))), '%')
                            OR TRIM(LOWER(emp.Industry)) LIKE CONCAT('%', TRIM(LOWER(a.Course)), '%')
                            OR TRIM(LOWER(a.Course)) LIKE CONCAT('%', TRIM(LOWER(emp.Industry)), '%')
                        )
                        -- Ensure Email check happens only when there is matching alumni data
                        AND (a.Email = '{$email}' OR a.Email IS NULL)
                        -- Exclude closed jobs based on Job_Close_Date
                        AND (ej.Job_Close_Date IS NULL OR ej.Job_Close_Date >= CURDATE())
                        -- Exclude closed jobs based on job position availability
                        AND (ej.Available_Positions IS NULL OR ej.Available_Positions != 0)
                    GROUP BY ej.ID
                    ORDER BY ej.Job_Start_Date DESC";
                ...
                while($row = mysqli_fetch_assoc($query_run))
                    {
            ?>
                <div class="col-lg-4 job-card" 
                data-job-title="<?php echo strtolower($row['Job_Title']); ?>" 
                 data-company-name="<?php echo strtolower($row['Company_Name']); ?>" 
                 data-city="<?php echo strtolower($row['City']); ?>"
                 data-state="<?php echo strtolower($row['State']); ?>"
                 data-salary="<?php echo strtolower($row['Salary']); ?>"
                 data-schedule-day="<?php echo strtolower($row['Sched_Day']); ?>"
                 data-job-type="<?php echo strtolower($row['Job_Type']); ?>">

                    <div class="card">
                      <div class="card-body">
                        <input type="hidden" name="job_id" id="job_id" value="<?php echo $row['ID']; ?>">

                        <input type="hidden" value="<?php echo $row['Job_Type']; ?>">
                        ...
                        <p class="card-text" id="descript-text"><?php echo $row['Description']; ?></p>
          ...
          <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab" tabindex="0">
            ...
            <?php
                ...
                $query = "SELECT * FROM tbl_employer_joblist
                    WHERE (Job_Close_Date IS NULL OR Job_Close_Date >= CURDATE())
                    -- Exclude closed jobs based on job position availability
                    AND (Available_Positions IS NULL OR Available_Positions != 0)
                    ORDER BY ID DESC";
...
<script>
    ...
    fetch('Alumni-Notif.php', {
        method: 'POST',
        body: formData
    })
        .then(response => response.text())
        .then(result => {
            alert('Application was Submitted, Apply more!');
...
function toggleClearButton() {
    ...
    if (input.value.length > 0) {
        clearButton.style.display = 'inline'; // Show clear button when there's input
    } else {
...
function filterJobs() {
    const searchTerm = $('#searchInput').val().toLowerCase();
        ...
        // Filter jobs in "Jobs For You" tab
        $('#jobsForYou .job-card').each(function() {
            const jobTitle = $(this).data('job-title');
            const companyName = $(this).data('company-name');
            const city = $(this).data('city');
            const state = $(this).data('state');
            const salary = $(this).data('salary');
            const scheduleDay = $(this).data('schedule-day');
            ...
            const matchesSearch = 
                ...
                scheduleDay.includes(searchTerm) ||
                jobType.includes(searchTerm);

            $(this).toggle(matchesSearch);
        });

        // Filter jobs in "Recent Jobs" tab
        $('#recentJobs .job-card').each(function() {
            const jobTitle = $(this).data('job-title');
            const companyName = $(this).data('company-name');
            const city = $(this).data('city');
            const state = $(this).data('state');
