...
if (isset($_POST['applybtn'])) {
    ...
    header("Location: Alumni-Archive.php");
    exit;
}
...
<div class="col-lg-12">
 ...
 <div class="profile_info1" style="position: relative;">
    <a href="Alumni-Applications.php" style="margin-top: -10px;"><span><i class='bx bx-chevron-left'></i></span> Return to Applications</a>
</div>


    <div class="jobtabs">
        <div class="tab-content" >
            <p>Check the list of jobs that have been declined by the Employer/Company you've applied with. Please note that these declined job records will remain in the archive for <b>14 days</b> before being permanently deleted. Be sure to review any necessary details before this time period expires.</p>
            <div class="row">
        <?php

        $query = "SELECT tbl_employer_joblist.*, tbl_employer_declined.Status,
            tbl_employer_declined.Note, 
            tbl_employer_declined.Declined_Date
          FROM tbl_employer_joblist
          INNER JOIN tbl_employer_declined
          ON tbl_employer_joblist.ID = tbl_employer_declined.Job_ID
          AND tbl_employer_joblist.Email = tbl_employer_declined.Email_Employer
          AND tbl_employer_declined.Email_Alumni = '{$email}'
          ORDER BY tbl_employer_declined.Declined_Date DESC";
                ...
                $originalDateTime = $row['Declined_Date'];
                    ...
                    <div class="card">
                      <div class="card-body">
                        ...
                        <p class="postedate text-muted" data-postdate="<?php echo date('Y-m-d', strtotime($row['Job_Start_Date'])); ?>"></p>
                         ...
                         <p class="status-text" style="background-color: #9B2626"><span>Status: </span><?php echo $row['Status']; ?></p>


                      </div>
                    </div>

                <div class="div-note" style="padding: 10px; background-color: #FFBEBE; border-radius: 10px; border: 1px solid #555;">
                    <p style="font-weight: 600; display: flex; justify-content: space-between;">Employer's Note: <span style="font-size: 12px;"><i class="fa fa-clock-o" aria-hidden="true"></i> <?php echo $formattedDateTime; ?></span></p>
                    <i><p style="font-size: 14px; margin-top: -10px;"><?php echo $row['Note']; ?></p></i>
                </div>
