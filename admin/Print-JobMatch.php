...
<head>
    ...
    <title>Alumni | Print Job Matching</title>
...
<div class="row">
    ...
    <div class="form-group" style="margin-top: 10px;">
        ...
        <form method="GET" id="filterForm">
            ...
            <a href="Print-JobAligned.php"><button type="button" class="hirebtn">Job Alignment <i class="fa fa-external-link-square" aria-hidden="true" style="color: #C2AE44;"></i></button></a>
        </form>
    </div>
</div>
...
<h2>Job Matching Report</h2>
<div class="graph" style="height: 400px; margin-top: -30px; display: flex; justify-content: center; align-items: center;">
<canvas id="myPieChart" width="500" height="500"></canvas>
</div>
 <p style="margin-top: -30px;">Job Matching Percentage Chart</p>
    ...
    <tr>
        ...
        <th style="border: 1px solid #ddd; padding: 8px; padding-top: 12px; padding-bottom: 12px; background-color: green; color: white;">Matched Job</th>
        ...
        <th style="border: 1px solid #ddd; padding: 8px; padding-top: 12px; padding-bottom: 12px; background-color: green; color: white;">Unmatched Job Alumni</th>
        <th style="border: 1px solid #ddd; padding: 8px; padding-top: 12px; padding-bottom: 12px; background-color: green; color: white;">Percentage</th>
    </tr>
<?php
...
// Apply batch year filter if specified
if ($filterYear !== 'all') {
    $query .= " WHERE a.Batch_Year = '$filterYear'";
}
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Extract program name (text before the hyphen)
        $display_name = explode('-', $course_name)[0];

        // Extract course key (before the hyphen)
        $courseHyphenPos = strpos(strtolower($course_name), '-');
        $course_key = ($courseHyphenPos !== false) ? trim(substr($course_name, 0, $courseHyphenPos)) : strtolower($course_name);

        // Extract job type key (after the hyphen)
        $jobTypeHyphenPos = strpos(strtolower($job_type), '-');
        $job_type_key = ($jobTypeHyphenPos !== false) ? trim(substr($job_type, $jobTypeHyphenPos + 1)) : strtolower($job_type);

        // Normalize spaces and remove extra symbols
        $course_key = preg_replace('/[^a-z0-9 ]/i', '', $course_key);
        $job_type_key = preg_replace('/[^a-z0-9 ]/i', '', $job_type_key);

        // Match check
        if (strpos($course_key, $job_type_key) !== false || strpos($job_type_key, $course_key) !== false) {
            $matched++;
        } else {
            $unmatched++;
        }

        // Count by course for totals
        if (!isset($course_totals[$display_name])) {
            $course_totals[$display_name] = ['matched' => 0, 'unmatched' => 0];
        }

        if (strpos($course_key, $job_type_key) !== false || strpos($job_type_key, $course_key) !== false) {
            $course_totals[$display_name]['matched']++;
        } else {
            $course_totals[$display_name]['unmatched']++;
        }
    }

    // Calculate total alumni for percentage computation
    $total_alumni = $matched + $unmatched;

    // Initialize overall counters for the final row
    $overall_matched = 0;
    $overall_unmatched = 0;

    // Display the results for each course
    $no = 1;
    foreach ($course_totals as $course_name => $totals) {
        $matched_count = $totals['matched'];
        $unmatched_count = $totals['unmatched'];

        $matched_percentage = $total_alumni > 0 ? ($matched_count / $total_alumni) * 100 : 0;
        $unmatched_percentage = $total_alumni > 0 ? ($unmatched_count / $total_alumni) * 100 : 0;
        ...
        <tr>
            ...
            <td style='border: 1px solid #ddd; padding: 8px;'>$matched_count</td>
            <td style='border: 1px solid #ddd; padding: 8px;'>" . number_format($matched_percentage, 2) . "%</td>
            <td style='border: 1px solid #ddd; padding: 8px;'>$unmatched_count</td>
            <td style='border: 1px solid #ddd; padding: 8px;'>" . number_format($unmatched_percentage, 2) . "%</td>
        </tr>";
        $no++;
    }

    // Add the grand total row
    $grand_total_percentage = $total_alumni > 0 ? ($overall_matched / $total_alumni) * 100 : 0;

    echo "
    <tr style='font-weight: bold; background-color: #f0f0f0;'>
        <td colspan='2' style='border: 1px solid #ddd; padding: 8px; text-align: center;'>Grand Total</td>
        <td style='border: 1px solid #ddd; padding: 8px;'>$overall_matched</td>
        <td style='border: 1px solid #ddd; padding: 8px;'>" . number_format($grand_total_percentage, 2) . "%</td>
        <td style='border: 1px solid #ddd; padding: 8px;'>$overall_unmatched</td>
        <td style='border: 1px solid #ddd; padding: 8px;'>" . number_format(($overall_unmatched / $total_alumni) * 100, 2) . "%</td>
    </tr>";
} else {
    // No results case
    echo "
    <tr>
        <td colspan='6' style='border: 1px solid #ddd; padding: 8px; text-align: center;'>No data available for the selected batch year.</td>
    </tr>";
}
...
<script>
let chart = null;  // To store chart instance
...
function initializePieChart() {
    ...
    let noDataMessage = contentDocument.getElementById('noDataMessage');
    ...
    const selectedYear = urlParams.get('filter_year') || 'All Batch Year'; // Default to 'All Batch Year' if no year selected

    // Fetch data from the backend
    fetch(`fetch_job_matching_chart.php?year=${selectedYear}`)
        ...
        .then(data => {
            if (data.total === 0) {
                ...
                // Calculate percentages based on counts and total
...
window.onload = initializePieChart;
