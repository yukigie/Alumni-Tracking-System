...
function validateImage($image, $allowedExtensions, $maxSize) {
    ...
    move_uploaded_file($tmpName, 'index/css/img/' . $newImageName);
...
if (isset($_POST['insertdata'])) {
    ...
    $check = mysqli_query($con, "SELECT * FROM tbl_event WHERE Event_Name='$Event'");
    
    if (mysqli_num_rows($check) > 0) {
        setAlert("Already Exist!", "The Event You Entered Is Already On The List!", "warning");
    } else {
        if ($_FILES["Image"]["error"] === 4) {
            ...
            if ($newImageName) {
                $query = "INSERT INTO tbl_event (Event_Name, Image, BatchYear, Schedule, Description, Course) VALUES ('$Event', '$newImageName', '$BatchYear','$Sched', '$Description', '$course')";
                
                if (mysqli_query($con, $query)) {
                    setAlert("Successfully Added!", "A New Event Has Been Successfully Added!", "success");
...
if (isset($_POST['updatedata'])) {
    $ID = $_POST['update_id'];
    $UP_Event = $_POST['UP_Event'];
    $UP_BatchYear = $_POST['UP_BatchYear'];
    $UP_Sched = $_POST['UP_Sched'];
    $UP_Description = $_POST['UP_Description'];
    $UP_course = $_POST['UP_course'];
    
    // Check if the title already exists in another record
    $check = mysqli_query($con, "SELECT * FROM tbl_event WHERE Event_Name='$UP_Event' AND Event_ID != '$ID'");
    
    if (mysqli_num_rows($check) > 0) {
        setAlert("Already Exist!", "The Event Name You Entered Is Already On The List!", "warning");
    } else {
        $newImageName = $_FILES["Image1"]["error"] === 4 ? mysqli_fetch_assoc(mysqli_query($con, "SELECT Image FROM tbl_event WHERE Event_ID = '{$ID}'"))['Image'] : validateImage($_FILES["Image1"], ['jpg', 'jpeg', 'png'], 10000000);

        if ($newImageName) {
            $query = "UPDATE tbl_event SET Event_Name = '{$UP_Event}', Image = '{$newImageName}', BatchYear = '{$UP_BatchYear}', Schedule = '{$UP_Sched}', Description = '{$UP_Description}', Course = '{$UP_course}' WHERE Event_ID = '{$ID}'";
            
            if (mysqli_query($con, $query)) {
                setAlert("Successfully Updated!", "Event has been successfully updated!", "success");
                header("Location: Events.php");
                    exit;
            } else {
                setAlert("Data Not Updated!", "Failed to update the Event.", "error");
                header("Location: Events.php");
                    exit;
            }
        }
...
if (isset($_POST['deletedata'])) {
    ...
    if ($con->query($sql)) {
        setAlert("Successfully Deleted!", "Event Is Successfully Deleted!", "success");
        header("Location: Events.php");
        exit;
    } else {
        setAlert("Data Not Deleted!", "Failed to delete the Event.", "error");
        header("Location: Events.php");
        exit;
    }
}
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="exampleModalLabel" style="font-weight: 600;">Add Event</h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <h5>Event Details</h5>

      <form action="Events.php" method="POST" enctype="multipart/form-data" class="form_body" autocomplete="off">

        <div class="modal-body">
          <div class="form-group">
            <label>Event Name:</label>
            <input type="text" name="Event" id="Event" class="form-control" placeholder="Enter Event Name" required>
          </div>
          ...
          <div class="row justify-content-center">
              ...
              <div class="form-group" style="margin-top: 20px;">
                ...
                <select name="BatchYear" id="BatchYear" class="form-control">
                  <option hidden selected value="">Select Batch Year</option>
                   <option value="All Batch">All Year</option>
                  <?php
                    $currentYear = date("Y");
            ...
            <div class="col-lg-6 col-md-12">
              <div class="form-group" style="margin-top: 20px;">
                ...
                <input type="datetime-local" name="Sched" id="Sched" class="form-control" required>
              </div>
            </div>
...
<div class="modal fade" id="EditModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="exampleModalLabel" style="font-weight: 600;">Update Event</h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      ...
      <form action="Events.php" method="POST" enctype="multipart/form-data" class="form_body" autocomplete="off">
        <div class="modal-body">
          ...
          <div class="form-group">
            <label>Event Name:</label>
            <input type="text" name="UP_Event" id="UP_Event" class="form-control" required>
          </div>
          ...
          <div class="row justify-content-center">
                ...
                if ($result = mysqli_query($con, $sql)) {
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        <div class='col-lg-6'>
                            <div class='form-group' style='margin-top: 20px;'>
                                <label>Course Name:</label>
                                <select name='UP_course' id='UP_course' class='form-control'>
            ...
            <div class="col-lg-6 col-md-12">
              <div class="form-group" style="margin-top: 20px;">
                <label>Schedule Date & Time:</label>
                <input type="datetime-local" name="UP_Sched" id="UP_Sched" class="form-control" required>
              </div>
            </div>
...
<div class="modal fade" id="DeleteModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="exampleModalLabel" style="font-weight: 600;">Delete Event</h4>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <h5>Confirm Message</h5>

      <form action="Events.php" method="POST" class="form_body" autocomplete="off">
        <div class="modal-body">
        ...
        <table class="table table-bordered" id="myTable" style="width: 100%;">
            <thead>
                <tr>
                    ...
                    <th scope="col" style="width: 170px;">Tools</th>                  
                </tr>
            </thead>
            <tbody>
                <?php
...
<script>
    ...
    document.getElementById('UP_Sched').setAttribute('min', currentDateTime);
    document.getElementById('Sched').setAttribute('min', currentDateTime);
</script>
