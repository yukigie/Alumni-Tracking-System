...
<head>
    ...
    <title>Alumni | Print Batch</title>
...
<div class="row">
        ...
        <form method="GET" id="filterForm">
            ...
            <div class="col-lg-12">
              <div class="form-group" style="margin-top: 20px;">
                <div name="textarea" id="default" class="form-control" rows="3">
    ...
    <?php
        ...
        // Fetch the most current year if "All Batch Years" is selected
        if ($selectedYear === 'all') {
            $currentYearQuery = "SELECT MAX(Batch_Year) AS max_year FROM tbl_alumni";
            $currentYearResult = $con->query($currentYearQuery);
            $mostCurrentYear = ($currentYearResult && $currentYearResult->num_rows > 0) 
                ? $currentYearResult->fetch_assoc()['max_year'] 
                : 'N/A'; // Fallback if no data is found
        } else {
            $mostCurrentYear = $selectedYear;
        }

        // Determine if leastYear should be hidden
        $leastYearStyle = ($selectedYear === 'all') ? '' : 'display: none;';
    ?>

<h2>Graduates of CvSU Imus Year <span style="<?php echo $leastYearStyle; ?>">2008 - </span><span id="currentYear"><?php echo $mostCurrentYear; ?></span></h2>
...
<canvas id="myPieChart" width="500" height="500"></canvas>
 ...
 <p style="margin-top: -30px;">Alumni Percentage By Course Chart</p>
    ...
    <table id="alumni-tbl" style="border-collapse: collapse; width: 100%; margin-top: 70px;">
        <tr>
            ...
            <th style="border: 1px solid #ddd; padding: 8px; padding-top: 12px; padding-bottom: 12px; background-color: green; color: white;">Total of Graduates</th>
        ...
        $sql = "SELECT Course, COUNT(*) AS total_graduates 
                FROM tbl_alumni 
                $whereClause
                GROUP BY Course 
                ORDER BY Course ASC";
        ...
        // Calculate total graduates for percentage computation
        $total_query = "SELECT COUNT(*) AS total_graduates FROM tbl_alumni $whereClause";
        $total_result = $con->query($total_query);
        $grand_total = ($total_result && $total_result->num_rows > 0) ? $total_result->fetch_assoc()['total_graduates'] : 1; // Avoid division by 0

        $overall_percentage = 0; // To accumulate total percentage
        $overall_graduates = 0;  // To accumulate total graduates
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $percentage = ($total_graduates / $grand_total) * 100;

                // Accumulate totals
                $overall_graduates += $total_graduates;
                ...
                <tr>
                    ...
                    <td style='border: 1px solid #ddd; padding: 8px;'>$total_graduates</td>
                    <td style='border: 1px solid #ddd; padding: 8px;'>" . number_format($percentage, 2) . "%</td>
                </tr>";
                $no++;
            }
            ...
            <tr style='font-weight: bold; background-color: #f0f0f0;'>
                <td style='border: 1px solid #ddd; padding: 8px; text-align: center;' colspan='2'>Grand Total</td>
                <td style='border: 1px solid #ddd; padding: 8px;'>$overall_graduates</td>
                <td style='border: 1px solid #ddd; padding: 8px;'>" . number_format($overall_percentage, 2) . "%</td>
            </tr>";
        } else {
            echo "<tr><td colspan='4' style='border: 1px solid #ddd; padding: 8px; text-align: center;'>No records found</td></tr>";
        }
...
function initializePieChart() {
    ...
    noDataMessage = contentDocument.getElementById('noDataMessage');
    ...
    fetch(`fetch_course_chart.php?year=${selectedYear}`)
